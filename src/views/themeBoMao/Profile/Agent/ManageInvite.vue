...
#agent-manage
  .page-tabs
    ...
    router-link(:to="{path: '/profile/agent/manageIcode'}") 邀请码管理
  #manage-invite
    Form(ref="form" :model="form")
      .filter
        FormItem.quick-date(label="开户类型：")
      ...
      .main
        FormItem(v-for="(item, index) in list" :key="index" :label="item.label" :prop="'points.' + item.id" :rules="{required: true, message: '返点不能为空'}")
          Input(v-model="form.points[item.id]" @on-blur="onBlur(item.id, item, $event.target)")
          span.help(v-text="`（自身返点${item.point}，可为下级设置返点范围${item.minPoint}-${item.maxPoint}）`")
      FormItem
        Button(:loading="loading" @click="handleSubmit('form')") 生成邀请码
    Alert(type="warning")
      h3 ※ 温馨提示：
  ...
  methods: {
    ...
    submit() {
      ...
      agentApi.setInviteUrl(this.form).then(data => {
        if (data.code != 0) return this.ivNotice(data.msg || '设置失败', 'warning');
        this.modal = true;
        this.$Modal.info({
          title: '温馨提示',
