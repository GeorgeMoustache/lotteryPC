...
#agent-manage
  ...
  #manage-icode
    ...
    .table-page(v-if="list.length")
      Page(:total="total" :current="query.current" :page-size="query.size" @on-change="changePage" size="small" placement="top" show-elevator show-total)
    button(style="display:none;" ref="copybtn" v-clipboard="copyData" @success="handleSuccess" @error="handleError") 复制
    Alert.alert(type="warning" show-icon) 温馨提示：“邀请码” 与 “注册链接” 功能一致，可以复制邀请码、注册链结，或直接复制二维码转发注册链结。
    Modal(v-model="infoModal")
      h3(slot="header") 详情
      Form(:label-width="120")
        FormItem(v-for="(item, index) in infoList" :key="index" :label="item.label")
          Input(:value="item.setPoint" :readonly="true" :disabled="true")
    //- 代理會員註冊
