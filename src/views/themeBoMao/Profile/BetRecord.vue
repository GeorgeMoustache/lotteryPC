...
#bet-record
  ...
  .filter
    ...
    .option(v-if="tabIndex == -1") 选择彩种：
      ...
      Select(v-model="query.betType")
        Option(v-for="(item, index) in ['全部投注', '普通投注', '追号投注']" :value="index" :label="item" :key="index")
    //- 第三方用篩選項目
    .option(v-else) 平台选择：
      Select(v-model="query.platId")
        Option(value="9999") 全部
        Option(v-for="item in platList" :value="item.platId" :key="item.platId" :label="item.showName")
    .submit
      Button(type="info" @click="searchData()" :loading="submiting") 开始查询
      | (可查询至多两个月记录)
  //- 當日詳情 (彩票)
  ul.today-detail(v-if="tabIndex == -1") 
    li 当日详情：
      span(v-text="today.total")
    li 中奖金额：
      span(v-text="today.win")
    li 盈利金额：
      span(v-text="today.profit")
    li 详情数据每10分钟统计一次，以明细为准
  //- 當日詳情 (第三方)
  ul.today-detail(v-else)
    li 输赢统计：
      span(:class="amountProfit > 0 ? 'text-success': 'text-error'" v-text="amountProfit") 
    li 可检视近两个月记录，资料五分钟统计一次
  //- 資料列表
...
import Modal4BetDetail from "@/components/themeBoMao/Modal4BetDetail";
...
export default {
  ...
  methods: {
    ...
    fetchData () {
      ...
      this.tabList.forEach(item => {
        if (item.id == this.tabIndex) {
          this.$parent.currentPath = item.name;
        }
      })
    },
    //換頁
  ...
  watch: {
    $route(to) {
      this.selectTab(this.$route.params);
    }
  }
}
</script>