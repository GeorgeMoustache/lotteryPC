...
.bet-bar(:class="hackCls")
  ...
  template(v-if="/DXDS$/.test(mode)")
    ...
    RadioGroup.cell-filter(v-model="filter" @on-change="filterChange")
      Radio.none-radio.fn-vishide(:label="0" style="width: 9px;")
        span.num-ball.ball-sm
  ...
  //- 任选 直选和值、组选和值、组选复式
  template(v-else-if="/^RX[0-9][A-Z0-9]{4,}/.test(mode)")
    .top-digits(v-if="index == 0")
    ...
    template(v-if="/HZ$/.test(mode)")
      .bet-bar-inner
        ...
        .cell-main
          CheckboxGroup.check-group(v-model="betNum")
            template(v-for="(n, i) in betInfo.nums" v-if="i <= infoLenHalf")
              Checkbox.none-checkbox(:label="n")
                span.num-ball(v-text="n")
          span.num-ball.fn-vishide(v-if="isOdd" style="float: none;")
      .bet-bar-inner
        .cell-side.fn-vishide(v-text="digit")
        .cell-main
          CheckboxGroup.check-group(v-model="betNum")
            template(v-for="(n, i) in betInfo.nums" v-if="i > infoLenHalf")
              Checkbox.none-checkbox(:label="n")
...
export default {
  ...
  computed: {
    ...
    hackCls() {
      // 任选，和值不需要背景色
      const { mode } = this;
      const isHack = /^RX[0-9][A-Z0-9]{4,}/.test(mode) || /Z[A-Z]XHZ$/.test(mode);
      return {
        'hack-background': isHack,
      };
    },
  },
  created() {
