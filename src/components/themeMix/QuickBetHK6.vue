      ...
      tbody
        tr(v-for="i in 10")
          template(v-for="j in 5")
            td(
              v-if="list[i-1+(j-1)*10]"
              :key="list[i-1+(j-1)*10].val"
              :class="getCls(list[i-1+(j-1)*10])"
              @click="toggleSelect(list[i-1+(j-1)*10])"
              ) {{renderVal(list[i-1+(j-1)*10])}}
    .filter-right
              ...
              tr(v-for="(child, child_idx) in twsLabel", :key="'tws' + child_idx")
                td
                  Checkbox(v-for="(item, item_idx) in child", :key="item.value", :label="item.value")
                    span.label {{item.label}}
  .modal-footer
    Button(type="text" size="large" @click="cancel") 取消
