...
.pay-model
  Form(
    ...
    FormItem(v-if="needProve" label="真实姓名:" prop="prove")
      Input.form-width(v-model="form.prove")
      span.text-warning * 请输入付款人真实姓名
    ...
    FormItem(label="微信昵称信息:")
      dl.select-info
        dd(v-if="src")
          img(:src="src" width="200")
        dd 收款帐号：{{form.accountNo}}
        //- 中彩特别处理
        dd(v-if="host == 'zc'") 收款人：{{form.accountName}}
        dd(v-else) 收款人：{{form.accountName}}（可转账发红包）
        dd 请您添加公司微信为好友，进行转账或红包。完成之后请将会员账号发送至我们的微信上。
    FormItem
  ...
  data() {
    return {
      ...
      rules: {
        ...
        prove: [{ required: true, message: '请输入真实姓名' }],
