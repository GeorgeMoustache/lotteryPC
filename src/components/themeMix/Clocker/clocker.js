...
function strftime(offsetObject) {
  return function(format) {
    ...
    if (directives) {
      var showDay = directives.find(el => el.indexOf('D') > -1) // fix
      for (var i = 0, len = directives.length; i < len; ++i) {
        ...
        if (DIRECTIVE_KEY_MAP.hasOwnProperty(directive)) {
          ...
          if (!showDay && value == 'hours') { // fix
            value = Number(offsetObject['days'] * 24 + offsetObject[value])
          } else {
      ...
      // fix tpl的问题
      directives = format.match(/%_([a-zA-Z]{1})/gi);
      if (directives) {
        var showDay = directives.find(el => el.indexOf('D') > -1);
        var showHours = directives.find(el => el.indexOf('H') > -1);
        var days = offsetObject['days'];
        if (days > 0 && !showDay && showHours) {
          var hours = Number(days * 24 + offsetObject['hours']);
          offsetObject.hours = hours;
          hours = hours + '';
          offsetObject.hours_1 = hours.charAt(0);
          offsetObject.hours_2 = hours.charAt(1);
        }
      }
    }
...
var fns = {
  ...
  getOffset() {
    let startTime = new Date().setMilliseconds(window.diffTime)
    this.totalSecsLeft = this.finalDate.getTime() - startTime // In miliseconds
