<template lang="pug">
div
  Modal.modal-bet-detail(v-model="modal" title="注单详情")
    .order-no(v-text="`注单号：${info.no}`")
    table.modal-table
      tr
        th 账号：
      ...
      tr
        ...
        td(colspan="3" v-if="info.betNum")
          template(v-if="info.betNum.length > 100")
            p.nums(:class="{ellipsis: !info.showMore}") {{info.betNum}}
            a.toggle-nums(@click="toggleDSDetails(info)") {{info.viewMoreText}}
          template(v-else)
  ...
  methods: {
    ...
    sendRedpack() {
      ...
      chatApi.sendRedpack(form).then(res => {
        ...
        //博貓模板發送成功後需開啟聊天室
        this.$parent.$parent.$parent.webChatEnabled = true;
      });
    },
    //關閉發紅包
    closeSendRedpack() {
