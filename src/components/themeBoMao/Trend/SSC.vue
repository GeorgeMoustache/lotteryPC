    ...
    thead
      tr
        ...
        template(v-for="n in colLen")
          th(:colspan="digitLen" v-text="`${digit.charAt(n-1)}位`")
      tr
        template(v-for="i in colLen")
          th.dark(v-for="j in digitLen" v-text="j-1")
    tbody
      tr.ball-row(v-for="(item, index) in list" :key="index")
        ...
        template(v-for="(omit, idx) in item.omits")
          td.light-bg(v-for="(n, i) in omit.split(',')")
            span(v-text="+n || i" :class="fomartCls(n, idx)")
      tr
        th 出现总次数
        td(:colspan="colLen")
  ...
  methods: {
    init () {
      this.$store.dispatch('trendData', this.params).then(() => {
        ...
        this.tid = setTimeout(UTIL.init({colLen: this.colLen}, '#ffd100'), 200, {colLen: this.colLen})
      })
    },
    fomartCls(n, i) {
      let cls = 'text-ball'
