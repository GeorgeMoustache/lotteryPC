...
.bet-bar-PCDD
  ...
  //- 波色
  template(v-else-if="/BSBS/.test(mode)")
    table
      thead
        tr
          td 选项
      ...
      tbody
        tr(v-for="(item, i) in BSBS")
          ...
          td
            price-input(:price.sync="item.price" :curObj="item" @formatPrice="formatPrice(item)")
  //- 豹子
  template(v-else-if="/BZBZ/.test(mode)")
    table
      thead
        tr
          td 选项
      ...
      tbody
        tr(v-for="(item, i) in BZBZ")
          ...
          td
            price-input(:price.sync="item.price" :curObj="item" @formatPrice="formatPrice(item)")
  //- 特码
  template(v-else-if="/TMTM/.test(mode)")
    table
      thead
        tr
    ...
    table.special3
      tr
        ...
        td(v-for="n in 3" v-if="TMTMB3.values")
          Select(v-model="TMTMB3.values[n-1]" :class="formatCls(TMTMB3.values[n-1])" @on-change="selectChange" :transfer="true")
            Option(v-for="n in 28" :value="n-1" :key="n" v-text="n-1" :label="n - 1")
        td 下注金额：
          price-input.price(:price.sync="TMTMB3.price" :curObj="TMTMB3" @formatPrice="formatPrice(TMTMB3)")
          | 元
</template>
